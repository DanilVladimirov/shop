def viewed_products(data):
    ...
    for i in data.values():
        responce_html += (
            ...
            f'<h5 class="card-title"><a href="/product/{i.get("id")}">{i["title"]}</a></h5>'
            f'</div></div>'
        )
...
def recommend_products(data):
    ...
    for i in data:
        responce_html += (
            f'<div class="card" style="width: 18rem;">'
            f'<div class="card-body">'
            f'<img width="100px" src="{i.photo.url}">'
            f'<h5 class="card-title"><a href="/product/{i.id}">{i.title}</a></h5>'
...
def buy_together(data):
    ...
    for i in data:
        responce_html += (
            f'<div class="card" style="width: 18rem;">'
            f'<div class="card-body">'
            f'<h5 class="card-title"><a href="/product/{i["product"]}"> {i["product__title"]}</a></h5>'
            f'<p class="card-text">Bought together {i["all_qty"]} times</p>'
            f'</div></div>'
        )
    return responce_html
...
def select_rating_product(id, select_rating):
    ...
    html_result += '</select>'
    html_result += f'<button onclick="edit_ratign({id})">vote</button>'
    return html_result
...
def delivery_np(data):
    ...
    if data.get('type') == 'region_novaposhta':
        html_result += f'<button type=\'button\' class = \'{data.get("type")}_delivery\' onclick="select_delivery(\'region\', \'novaposhta\')">Choose</button></span>'
    if data.get('type') == 'city_novaposhta':
        html_result += f'<button type=\'button\' class = \'{data.get("type")}_delivery\' onclick="select_delivery(\'city\', \'novaposhta\')">Choose</button></span>'
    if data.get('type') == 'warehouses_novaposhta':
        html_result += f'<button type=\'button\' class = \'{data.get("type")}_delivery\' onclick="select_delivery(\'warehouses\', \'novaposhta\', \'{data.get("ref")}\')">Choose</button></span>'
